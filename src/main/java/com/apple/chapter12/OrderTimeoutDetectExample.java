 ...
 * <p>
 * Created by wushengran on 2021/8/28  15:31
 */
public class OrderTimeoutDetectExample {
    public static void main(String[] args) throws Exception{
        ...
        // 1. 读取订单事件流
        KeyedStream<OrderEvent, String> orderStream = env.fromElements(
        ...
        // 4. 定义一个侧输出流标签，用来标识超时侧输出流
        OutputTag<String> timeoutTag = new OutputTag<String>("timeout") {};

        // 5. 对复杂事件进行提取处理
        SingleOutputStreamOperator<String> result = patternStream.process(new OrderPayMatchProcess());

        // 老版本写法
        SingleOutputStreamOperator<String> result2 = patternStream.flatSelect(timeoutTag,
                new PatternFlatTimeoutFunction<OrderEvent, String>() {
                    ...
                    public void timeout(Map<String, List<OrderEvent>> map, long l, Collector<String> collector) throws Exception {
                        ...
                        collector.collect("订单 " + createEvent.orderId + " 超时未支付！用户：" + createEvent.userId);
                    }
                },
        ...
        result2.print("payed");
        result2.getSideOutput(timeoutTag).print("timeout");
    ...
    // 自定义实现PatternProcessFunction
    public static class OrderPayMatchProcess extends PatternProcessFunction<OrderEvent, String>
    implements TimedOutPartialMatchHandler<OrderEvent>{
        ...
        public void processTimedOutMatch(Map<String, List<OrderEvent>> map, Context context) throws Exception {
            OrderEvent createEvent = map.get("create").get(0);
            context.output(new OutputTag<String>("timeout") {}, "订单 " + createEvent.orderId + " 超时未支付！用户：" + createEvent.userId);
        }
    }
}
